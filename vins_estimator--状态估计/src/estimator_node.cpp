...
void process()
{
    ...
    {
        std::vector<std::pair<std::vector<sensor_msgs::ImuConstPtr>, sensor_msgs::PointCloudConstPtr>> measurements;
		//< ImuConstPtr  , PointCloudConstPtr  >  读取测量值
		
        std::unique_lock<std::mutex> lk(m_buf);
        con.wait(lk, [&]
                 {
            return (measurements = getMeasurements()).size() != 0;  //主要功能用于读取数据
			
			    //getMeasurements()函数代码
				/*std::vector<std::pair<std::vector<sensor_msgs::ImuConstPtr>, sensor_msgs::PointCloudConstPtr>>
					getMeasurements()
					{
						std::vector<std::pair<std::vector<sensor_msgs::ImuConstPtr>, sensor_msgs::PointCloudConstPtr>> measurements;
        ...
        for (auto &measurement : measurements)//auto：用来声明自动变量。它是存储类型标识符，表明变量（自动）具有本地范围，
											  //块范围的变量声明（如for循环体内的变量声明）默认为auto存储类型。
        {
				...
				/*void send_imu(const sensor_msgs::ImuConstPtr &imu_msg)
						{
							...
							estimator.processIMU(dt, Vector3d(dx, dy, dz), Vector3d(rx, ry, rz));  //
									/// processIMU void Estimator::processIMU(double dt, const Vector3d &linear_acceleration, const Vector3d &angular_velocity)
											{
												if (!first_imu)
												{
													first_imu = true;
            ...
            {
                int v = img_msg->channels[0].values[i] + 0.5;  //？？lkp
...
int main(int argc, char **argv)
{
    ...
    std::thread measurement_process{process}； //开启单独线程
